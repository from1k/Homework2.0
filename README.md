...
0. Перед началом выполнения лабораторной работы следует настроить окружение в котором собираетесь работать. Это можно осуществить используя команды, указанные в [Tutorial](https://github.com/tp-labs/lab02)

1. Создаем репозиторий в котором собираемся работать. Осуществить это можем двумя способами.
+ 1 способ. С помощью команды ```git clone``` копируем  заранее созданный пустой репозиторий на GitHub.
> ```bash
> $ git clone https://github.com/${GITHUB_USERNAME}/Homework2
> ```

+ 2 способ. С помощью набора команд создаем локальный репозиторий и связываем его с удаленным (уже существующим на GitHub).
> ```bash
> $ mkdir Homework1
> $ cd Homework2
> $ git init
> $ git remote add origin https://github.com/${GITHUB_USERNAME}/Homework2
> ``` 
</details>
